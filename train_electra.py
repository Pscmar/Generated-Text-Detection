...
class ModelTrainer:
    def __init__(
        ...
        self.model = ElectraForSequenceClassification.from_pretrained(model_name).cuda()
...
### ↓↓↓ need to be modified when you change the dataset ↓↓↓
                # torch.save(self.model.state_dict(), "best_model_electra.pt")
                torch.save(self.model.state_dict(), "best_model_electra_short.pt")
                # torch.save(self.model.state_dict(), "best_model_electra_mixed.pt")
...
### ↓↓↓ need to be modified when you change the dataset ↓↓↓        
        # self.model.load_state_dict(torch.load("best_model_electra.pt"))
        self.model.load_state_dict(torch.load("best_model_electra_short.pt"))
        # self.model.load_state_dict(torch.load("best_model_electra_mixed.pt"))
...
### ↓↓↓ need to be modified when you change the dataset ↓↓↓ 
        # "dataset/train.json", "dataset/val.json", "dataset/test.json"# train on original data
        "dataset/short_train.json", "dataset/short_val.json", "dataset/short_test.json", num_epochs=10 # train on short-sentence data
        # "dataset/mixed_train.json", "dataset/mixed_val.json", "dataset/mixed_test.json", num_epochs=10 # train on short-sentence data
    )
